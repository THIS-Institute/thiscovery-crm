...
def add_template_to_ddb(
    template_id, template_name, template_type, formatted_custom_properties, preview_url
):
    ddb_client = Dynamodb()
    ddb_client.put_item(
...
def main():
    ...
    input(
        "Please paste the template text in file hubspot_email_template_text.txt, save it and press return to continue."
    )
    ...
    if not custom_properties:
        confirmation = input(
            f"No custom properties found in preview page. Are you sure you want to add this template to Dynamodb? (y/n)"
        )
        if confirmation not in ["y", "Y"]:
            print("Import aborted")
            sys.exit(0)
    formatted_custom_properties = format_custom_properties(
    ...
    template_type = input(
        "Please enter the type of the new template (or leave blank to use the default type 'transactional-email-template'):"
    )
